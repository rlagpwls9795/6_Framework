...
public class MemberServiceImpl implements MemberService{
	...
	public Member login(Member inputMember) {
		...
		// 2. 입력 받은 비밀번호(평문)
		//    조회한 암호화된 비밀번호 비교
		//    -> BCryptPasswordEncoder.matches(평문, 암호문) 이용 (같으면 true, 아니면 false)
	...
	* - 선언적 트랜잭션 처리 2가지
	* 1) <tx:adcice>를 이용한 xml 작성 방법
	* 2) @Transactional 어노테이션 작성 방법
	*	- 클래스, 메서드 위에 작성 가능
		- 조건 : 1. AOP를 이용한 기술 -> Service Interface 필요
			   2. 트랜잭션 매니저가 bean으로 등록되어 있어야 한다. (root-context.xml)
	* - @Transactional 어노테이션 특징
	* 	- 예외가 발생한 경우에 rollback 자동 진행
	* 	- 예외의 기본값은 RuntimeException
	* 		-> SQL 진행 시 발생하는 예외 == SQLException(RuntimeException 형제)
	* 									(다형성 적용 X)
	* 
	*       -> 다른 예외에도 rollback이 진행되고 싶다면 rollbackFor = 예외클래스 작성
	*/
