...
@Aspect // 공통 관심사(AOP) + 포인트컷이 작성된 클래스임을 명시
public class BeforeAdviceAspect {

	// 로그 출력 객체 얻어 오기
	private Logger logger = LoggerFactory.getLogger(BeforeAdviceAspect.class);

	// CommonPointcut 클래스 메서드 중 serviceImplPointcut() 메서드에 작성된 pointcut 얻어 옴
	@Before("CommonPointcut.serviceImplPointcut()")
	public void beforeServiceLog(JoinPoint jp) {
		// JoinPoint 객체 : 부가 기능이 적용되는 대상 객체의 클래스명, 메서드명, 파라미터 등을 얻을 수 있음

		String str = "===================================================";

		// jp.getTarget() : pointcut으로 지정된 대상 객체
