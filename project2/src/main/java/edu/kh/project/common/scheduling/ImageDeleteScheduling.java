...
//		System.out.println("0초마다 출력");
		...
		File[] arr = new File(folderPath).listFiles(); // 지정된 경로에 존재하는 파일 목록을 배열로 반환
		List<File> fileList = Arrays.asList(arr); // 배열 --> List로 변환
		
		// 3. 둘을 비교하여 Server 이미지 목록 중 DB에 없는 이미지 삭제
		if(!fileList.isEmpty()) { // 서버에 파일이 있는 경우
			
			for(File file : fileList) {
 				// file.toString(); // 실제 파일이 저장된 경로 반환
				// 문자열.subString(인덱스) : 지정된 인덱스부터 끝까지 잘라서 반환

//				String fileName = file.toString().substring(file.toString().lastIndexOf("\\") +1);
				String fileName = file.getName();
				
				// dbList에서 fileName과 일치하는 파일명이 없다 == 서버에는 있고 DB에는 없는 파일
