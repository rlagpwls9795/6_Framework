    ...
    inputImage[i].addEventListener("change", event =>{
        ...
        if(event.target.files[0] !=undefined){ // 선택된 파일이 있을 경우
            ...
            reader.onload = e=>{ // 파일을 다 읽어 온 후
                ...
                // 미리보기가 추가됨 == 파일이 추가됨 == 삭제하면 안됨 == deleteSet에서 해당 순번 제거
                deleteSet.delete(i);
            };
...
function updateValidate(){
    ...
    // deleteSet의 값을 "0,1,2" 형태로 변환하여 form 태그 내부 input 태그로 추가하여 같이 제출하게 만들기
    // Array.from(deleteSet) -> deleteSet을 배열로 변환

    // JS 배열 특징 : 배열을 HTML요소 또는 console에 출력하면 요소,요소,요소 형태의 문자열로 변환
